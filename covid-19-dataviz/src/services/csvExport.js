class CSVExportService {
    ...
    exportCovidData(confirmedData, deathsData, selectedCountries, globalStats) {
      const csvData = this.prepareCovidCSVData(confirmedData, deathsData, selectedCountries, globalStats)
    ...
    prepareCovidCSVData(confirmedData, deathsData, selectedCountries, globalStats) {
      ...
      selectedCountries.forEach(country => {
        ...
        if (deathsData && deathsData.data) {
          const deathCountryData = deathsData.data.filter(
            row => row['Country/Region'] === country
          )
    ...
    prepareCountryCSVData(confirmedData, deathsData, selectedCountries) {
      ...
      selectedCountries.forEach(country => {
        ...
        // Taux de mortalité
        const mortalityRate = totalConfirmed > 0 ? ((totalDeaths / totalConfirmed) * 100).toFixed(2) : '0.00'
    ...
    prepareGlobalStatsCSV(globalStats) {
      ...
      return [
        ...
        ['Taux de mortalité (%)', ((globalStats.deaths / globalStats.confirmed) * 100).toFixed(2), 'Calculé'],
        ['Taux de guérison (%)', ((globalStats.recovered / globalStats.confirmed) * 100).toFixed(2), 'Estimé'],
